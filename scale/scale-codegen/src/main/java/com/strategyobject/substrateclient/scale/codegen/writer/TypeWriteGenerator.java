...
public class TypeWriteGenerator extends TypeTraverser<CodeBlock> {
    private final ProcessorContext context;
    private final Map<String, Integer> typeVarMap;

    public TypeWriteGenerator(Class<CodeBlock> clazz,
    ...
    protected CodeBlock whenTypeVar(@NonNull TypeVariable type, TypeMirror _override) {
        return context.isSubtypeOfScaleSelfWritable(type) ?
                CodeBlock.builder()
                        .add("registry.resolve($T.class)", context.getScaleSelfWritableRawType())
                        .build() :
                CodeBlock.builder()
                        .add("writers[$L]", typeVarMap.get(type.toString()))
                        .build();
    }
