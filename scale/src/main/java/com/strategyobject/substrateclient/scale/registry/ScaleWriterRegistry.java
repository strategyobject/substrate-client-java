...
public final class ScaleWriterRegistry {
    ...
    private void autoRegistration() {
        Scanner.getSubTypesOf(ScaleWriter.class).forEach(writer -> {
    ...
    public ScaleWriter<?> resolve(@NonNull Class<?> clazz) throws ScaleWriterNotFoundException {
        val result = writers.get(clazz);
        if (result == null) {
            throw new ScaleWriterNotFoundException(clazz);
        }
