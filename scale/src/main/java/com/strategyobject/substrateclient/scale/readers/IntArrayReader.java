...
public class IntArrayReader implements ScaleReader<int[]> {
    @Override
    public int[] read(@NonNull InputStream stream, ScaleReader<?>... readers) throws IOException {
        Preconditions.checkArgument(readers == null || readers.length == 0);

        val len = CompactIntegerReader.readInternal(stream);
        val result = new int[len];
        val src = Streamer.readBytes(len * 4, stream);
