...
import com.strategyobject.substrateclient.storage.*;
...
import static com.strategyobject.substrateclient.pallet.Constants.*;
...
class StorageProcessor extends PalletMethodProcessor {
    ...
    private CodeBlock resolveHashingAlgorithm(AnnotationMirror keyAnnotation,
        ...
        if (hasherName.equals(StorageHasher.Blake2B128Concat.toString())) {
            builder.add("$T.$L()",
                    Blake2B128Concat.class,
                    BLAKE_2_128_CONCAT_INSTANCE);
        }

        if (hasherName.equals(StorageHasher.TwoX64Concat.toString())) {
            builder.add("$T.$L()",
                    TwoX64Concat.class,
